...
#{ Accessor methods.
	...
	def getController(self):
		"""
		Returns the controller of this Node. Default is C{None}.

		@return: The controller of this Node.
		@rtype: L{AbstractController} (or C{None} if not defined).
		"""
		return self._controller

	def setController(self, controller):
		"""
		Sets the controller of this Node for the MVC design pattern.

		@param controller: A L{AbstractController}.
		"""
	...
	def getDirector(self):
		...
		@rtype: L{Director} (or C{None} if not yet defined).
		"""
	...
	def setDirector(self, director):
		...
		@param director: the L{Director} of the application.
		"""
		self._director = director
		for child in self.getChildren():
			child.setDirector(director)
	...
	def setZOrder(self, zOrder):
		"""
		The Z-Order. Children are drawn in order relative to this. Default is C{0}.

		@param zOrder: C{int} (though C{float} is acceptable).
		"""
		self._zOrder = zOrder
	...
	def setVisible(self, isVisible):
		"""
		Whether or not this node & children are visible. Default is C{True}.

		@param isVisible: C{bool}.
		"""
	...
	def setUserData(self, userData):
		...
		@param userData: C{User defined}.
		"""
		self._userData = userData

	def getTag(self):
		"""
		A convenience string with which to identify this Node. Default is C{None}.

		@return: Convenience string.
		@rtype: C{string} (or C{None} if not defined).
		"""
		return self._tag
	...
	def setColor(self, color):
		"""
		Method for children who have color foregrounds for, e.g. tinting (like L{Sprite}s).
	...
	def getPosition(self):
		'''
		Get the current position of the Node. Note that the position is set relative to the C{Node's} parent.
	...
	def setPosition(self, position):
		'''
		Sets the current position of the Node. Note that the position is set relative to the C{Node's} parent.
...
#{ Children methods.
	...
	def getChildByTag(self, tag):
		"""
		Returns a child C{Node} whose tag matches the given C{string}.

		@param tag: The child's tag.
		@type tag: C{string}.
		@return: A child C{Node} if the child is found, C{None} otherwise.
		@rtype: C{Node} (or C{None} if the child is not found).
		"""
		for child in self.getChildren():
			if child.getTag() == tag:
				return child
	...
	def addChild(self, child, zOrder=None, tag=None):
		...
		child.setTag(tag)
		child._parent = self
		child.setDirector(self.getDirector())
		if self._isRunning:
			child.onEnter()
	...
	def _detachChild(self, child, shouldCleanup):
		...
		if shouldCleanup:
			child._cleanup()
		child._parent = None
		child.setDirector(None)
		self._children.remove(child)
#}

		
#{ Scale convenience methods.
	def getScale(self):
		"""
...
#{ Drawing methods.
	def draw(self, context):
		"""
		Subclass and do your drawing in here. The context will already be translated to the node's current position, so drawing to C{Point(0,0)} will actually render to L{getPosition}.
...
#{ Timer methods.
	def activateTimers(self):
		"""
		Activates any L{Timer}s that this Node has.
		"""
	...
	def deactivateTimers(self):
		"""
		Deactivates any L{Timer}s that this Node has.
		"""
		for callback in self._scheduledTimers:
			timer = self._scheduledTimers[callback]
			scheduler = self.getDirector().getScheduler()
...
#{ Action methods.
	...
	def stopAllActions(self):
		"""
		Stops all L{Action}s currently running on this Node.
		"""
		actionManager = self.getDirector().getActionManager()
	...
	def getActionByTag(self, tag):
		"""
		Gets a particular L{Action} currently running on this Node.
	...
	def numberOfRunningActions(self):
		"""
		Returns the number of L{Action}s currently running on this Node.
